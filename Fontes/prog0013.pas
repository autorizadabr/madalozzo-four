...
procedure TFormCadSerie.brnExcluirClick(Sender: TObject);
begin
...
procedure TFormCadSerie.btnFecharClick(Sender: TObject);
begin
  close;
end;

procedure TFormCadSerie.btnGravarClick(Sender: TObject);
var
  v_status : boolean;
begin
...
procedure TFormCadSerie.edCodiExit(Sender: TObject);
begin
...
procedure TFormCadSerie.edCodiPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  edcodi.Text:= consulta('cons_serie');
  edCodiExit(self);
  eddesc.SetFocus;
end;

procedure TFormCadSerie.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:= cafree;
  FormCadSerie:= nil;
end;

procedure TFormCadSerie.FormShow(Sender: TObject);
begin
  // carrega os dados do form
  lbTitu.Caption               := 'Série de Documento';
  Caption                      := '..::PROG0013::..';
  v_tabela                     := 'seri';
  v_esquema                    := 'cadastro';
...
procedure TFormCadSerie.quTableAfterCancel(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Navegando...';
end;

procedure TFormCadSerie.quTableAfterPost(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Navegando...';
end;

procedure TFormCadSerie.quTableBeforeEdit(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Editando registro...';
end;

procedure TFormCadSerie.quTableBeforeInsert(DataSet: TDataSet);
begin
