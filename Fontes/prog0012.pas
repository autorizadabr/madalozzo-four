...
type
  TFormCadTipoEntidade = class(TForm)
...
var
  FormCadTipoEntidade: TFormCadTipoEntidade;
...
procedure TFormCadTipoEntidade.brnExcluirClick(Sender: TObject);
begin
...
procedure TFormCadTipoEntidade.btnFecharClick(Sender: TObject);
begin
  close;
end;

procedure TFormCadTipoEntidade.btnGravarClick(Sender: TObject);
var
  v_status : boolean;
begin
...
procedure TFormCadTipoEntidade.edCodiExit(Sender: TObject);
begin
...
procedure TFormCadTipoEntidade.edCodiPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  edcodi.Text:= consulta('cons_tipoentidade');
  edCodiExit(self);
  eddesc.SetFocus;
end;

procedure TFormCadTipoEntidade.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:= cafree;
  FormCadTipoEntidade:= nil;
end;

procedure TFormCadTipoEntidade.FormShow(Sender: TObject);
begin
  // carrega os dados do form
  lbTitu.Caption               := 'Tipos de Entidade';
  Caption                      := '..::PROG0012::..';
  v_tabela                     := 'tien';
  v_esquema                    := 'cadastro';
...
procedure TFormCadTipoEntidade.quTableAfterCancel(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Navegando...';
end;

procedure TFormCadTipoEntidade.quTableAfterPost(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Navegando...';
end;

procedure TFormCadTipoEntidade.quTableBeforeEdit(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Editando registro...';
end;

procedure TFormCadTipoEntidade.quTableBeforeInsert(DataSet: TDataSet);
begin
