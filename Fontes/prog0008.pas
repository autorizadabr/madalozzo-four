...
var
  FormCadSubGrupo: TFormCadSubGrupo;
...
procedure TFormCadSubGrupo.brnExcluirClick(Sender: TObject);
begin
...
procedure TFormCadSubGrupo.btnFecharClick(Sender: TObject);
begin
  close;
end;

procedure TFormCadSubGrupo.btnGravarClick(Sender: TObject);
var
  v_status : boolean;
begin
...
procedure TFormCadSubGrupo.edCodiExit(Sender: TObject);
begin
...
procedure TFormCadSubGrupo.edCodiPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  edcodi.Text:= consulta('cons_subgrupo');
  edCodiExit(self);
  eddesc.SetFocus;
end;

procedure TFormCadSubGrupo.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:= cafree;
  FormCadSubGrupo:= nil;
end;

procedure TFormCadSubGrupo.FormShow(Sender: TObject);
begin
  // carrega os dados do form
  lbTitu.Caption               := 'Cadastro de Sub. Grupos';
  Caption                      := '..::PROG0008::..';
  v_tabela                     := 'sugr';
  v_esquema                    := 'cadastro';
...
procedure TFormCadSubGrupo.quTableAfterCancel(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Navegando...';
end;

procedure TFormCadSubGrupo.quTableAfterPost(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Navegando...';
end;

procedure TFormCadSubGrupo.quTableBeforeEdit(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Editando registro...';
end;

procedure TFormCadSubGrupo.quTableBeforeInsert(DataSet: TDataSet);
begin
