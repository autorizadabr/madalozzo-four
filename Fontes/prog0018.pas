...
procedure TFormGenero.brnExcluirClick(Sender: TObject);
begin
...
procedure TFormGenero.btnFecharClick(Sender: TObject);
begin
  close;
end;

procedure TFormGenero.btnGravarClick(Sender: TObject);
var
  v_status : boolean;
begin
...
procedure TFormGenero.edCodiExit(Sender: TObject);
begin
...
procedure TFormGenero.edCodiPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  edcodi.Text:= consulta('cons_genero');
  edCodiExit(self);
  eddesc.SetFocus;
end;

procedure TFormGenero.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:= cafree;
  FormGenero:= nil;
end;

procedure TFormGenero.FormShow(Sender: TObject);
begin
  // carrega os dados do form
  lbTitu.Caption               := 'Genero de Produtos';
  Caption                      := '..::PROG0018::..';
  v_tabela                     := 'gene';
  v_esquema                    := 'cadastro';
...
procedure TFormGenero.quTableAfterCancel(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Navegando...';
end;

procedure TFormGenero.quTableAfterPost(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Navegando...';
end;

procedure TFormGenero.quTableBeforeEdit(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Editando registro...';
end;

procedure TFormGenero.quTableBeforeInsert(DataSet: TDataSet);
begin
