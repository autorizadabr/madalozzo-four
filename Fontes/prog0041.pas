...
type
  TFormConsultaPagar = class(TForm)
    Image1: TImage;
    titulo: TcxLabel;
...
var
  FormConsultaPagar: TFormConsultaPagar;
...
procedure TFormConsultaPagar.btnConsultarClick(Sender: TObject);
begin
  sqlaux.close;
  sqlaux.sql.text:= 'select *, '+
                    ...
                    'and mofi_empr = :empr and mofi_loca = :loca and mofi_tipo = ''E'' ';
...
procedure TFormConsultaPagar.btnExportarClick(Sender: TObject);
begin
  if sd.execute then
   begin
...
procedure TFormConsultaPagar.btnFecharClick(Sender: TObject);
begin
  close;
end;

procedure TFormConsultaPagar.btnImprimirClick(Sender: TObject);
begin
  imprimir(rlreport1,RLBand4);
end;

procedure TFormConsultaPagar.cxGrid1DBTableView1CustomDrawCell(
...
procedure TFormConsultaPagar.edEntiExit(Sender: TObject);
begin
  edNomeEnti.Clear;
  if StrToIntDef(edEnti.Text,0) > 0 then
    begin
...
procedure TFormConsultaPagar.edEntiPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
procedure TFormConsultaPagar.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  cxGrid1DBTableView1.StoreToRegistry('prog0041');

  action:= cafree;
  FormConsultaPagar:= nil;
end;

procedure TFormConsultaPagar.FormShow(Sender: TObject);
begin
  ...
  caption:= '..::PROG0041::..';
  lbtitu.Caption:= 'Titutlos a Pagar';
...
procedure TFormConsultaPagar.RLDBText10BeforePrint(Sender: TObject;
