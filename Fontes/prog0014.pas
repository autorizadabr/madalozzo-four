...
type
  TFormUnidadeMedida = class(TForm)
...
var
  FormUnidadeMedida: TFormUnidadeMedida;
...
procedure TFormUnidadeMedida.brnExcluirClick(Sender: TObject);
begin
...
procedure TFormUnidadeMedida.btnFecharClick(Sender: TObject);
begin
  close;
end;

procedure TFormUnidadeMedida.btnGravarClick(Sender: TObject);
var
  v_status : boolean;
begin
...
procedure TFormUnidadeMedida.edCodiExit(Sender: TObject);
begin
...
procedure TFormUnidadeMedida.edCodiPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  edcodi.Text:= consulta('cons_unidade');
  edCodiExit(self);
  eddesc.SetFocus;
end;

procedure TFormUnidadeMedida.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action:= cafree;
  FormUnidadeMedida:= nil;
end;

procedure TFormUnidadeMedida.FormShow(Sender: TObject);
begin
  // carrega os dados do form
  lbTitu.Caption               := 'Unidade de Medida';
  Caption                      := '..::PROG0014::..';
  v_tabela                     := 'unid';
  v_esquema                    := 'cadastro';
...
procedure TFormUnidadeMedida.quTableAfterCancel(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Navegando...';
end;

procedure TFormUnidadeMedida.quTableAfterPost(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Navegando...';
end;

procedure TFormUnidadeMedida.quTableBeforeEdit(DataSet: TDataSet);
begin
  sb.Panels[0].text:= 'Editando registro...';
end;

procedure TFormUnidadeMedida.quTableBeforeInsert(DataSet: TDataSet);
begin
